...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $warranty_card_no = inputClean($_POST['warranty_card_no']);
    ...
    if (empty($warranty_card_no)) {
        sendResponse(0, 'Warranty Card No. is required.');
    }
    /*if (empty($query)) {
    ...
    try {
        if (!$stmt = $conn->prepare("INSERT INTO `warranty_claim`(`name`, `mobile`, `email`, `system_size`, `invoice_copy`, `warranty_card_no`, `created_at`) VALUES (?,?,?,?,?,?,?)")) {

            sendResponse(0, "Some error occured. code : 10");
        }

        if (!$stmt->bind_param("sssssss", $name, $mobile,  $email, $system_size, $invoice_copy_name, $warranty_card_no, $created_at)) {
        ...
        if ($stmt->execute()) {
            ...
            $message2 = "Hello Team ,<br ><br>
                        ...
                        Warranty Card No. : " . $warranty_card_no . " <br>
                        Query : " . $query . " <br>
